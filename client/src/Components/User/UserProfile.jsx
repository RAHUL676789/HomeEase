...
const UserProfile3 = () => {
  ...
  const buttonClass =
    ...
    const handleAutoDeleteBooking = async(isAutoDeleted)=>{
      try {
        const repsone = await axios.put(`/api/users/settings/${user?._id}`,{isAutoDeleted});
    ...
    <div className="max-w-6xl mx-auto h-[90vh] overflow-scroll no-scrollbar bg-white rounded-xl shadow-md shadow-gray-400 p-5 sm:p-8">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 pb-4 border-b">
        <div className="flex items-center gap-3">
          ...
          {/* Toggle Switch */}
          <div
            onClick={() => setisAutoDeleted((prev) => !prev)}
            className={`h-6 w-12 flex items-center px-1 rounded-full cursor-pointer transition-colors duration-500 ${
              isAutoDeleted ? "bg-teal-500" : "bg-gray-300"
            }`}
          >
            <div
              className={`h-4 w-4 rounded-full bg-white shadow-md transform transition-transform duration-500 ${
                isAutoDeleted ? "translate-x-6" : "translate-x-0"
              }`}
            ></div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {["pending", "accepted", "rejected", "cancelled", "completed"].map((btn, i) => (
              <button
                key={i}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {user?.bookings?.length ? (
              user.bookings.map((b, i) => (
                <div
                  key={i}
                  className={`border p-4 rounded bg-gray-50 hover:bg-white hover:shadow-md transition-all duration-300 ${b?.status === "expired" && "opacity-60"}`}
                >
                  ...
                  <p className="text-xs text-gray-500 mb-2">
                    Booked on:{" "}
                    {new Date(b?.createdAt).toLocaleDateString().split("/").join("-")}
                  </p>
                  <span
                    className={`text-xs sm:text-sm px-3 py-1 roundeD inline-block font-medium ${
                      b.status === "completed"
